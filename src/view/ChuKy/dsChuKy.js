...
export default function DsChuKy({ route, navigation }) {
  ...
  const getAPI = (search = '') => {
    fetch(`http://192.168.24.1/API_QuanLyNongTrai/ChuKy/getData.php?search=${search}`)
  ...
  const getAPINhanVien = (search = '', tenDangNhap = '') => {
    fetch(`http://192.168.24.1/API_QuanLyNongTrai/ChuKy/getDataBangMaNhanVien.php?search=${search}`)
      .then(response => response.json())
      .then(data => {
