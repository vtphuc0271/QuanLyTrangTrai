...
export default function GhiChepChuKy({ route, navigation }) {
  ...
  const handleAdd = async () => {
      try {
          const response = await fetch('http://192.168.24.1/API_QuanLyNongTrai/ChuKy/addDataLichGhiChep.php', {
            method: 'POST',
            headers: {
    ...
    <Modal
        ...
        <View style={styles.thongTinDV}>
          ...
          <View style={styles.view_TT_DV}>
            ...
            <View>
              ...
              <Text style={styles.data_TT_DV}> Nguồn gốc: {item.noiSanXuat}</Text>
              <Text style={styles.data_TT_DV}> Ngày nhập hàng: {item.ngayNhapHang}</Text>
              <Text style={styles.data_TT_DV}> Loại động vật: {item.loaiDongVat}</Text>
            </View>
          </View>
        </View>
      </View>
      <Text
       ...
       <View
        ...
        <TextInput
            style={{width:'auto',borderWidth:1,borderColor:'black',height:38,frontsize:15,}}
